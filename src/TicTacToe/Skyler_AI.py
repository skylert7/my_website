...
# ####################################################################################### #
#               Implementation of this AI is based on the paper:                          #
#                                                                                         #
#   Go-Moku and Threat-Space Search. L.V. Allis. H.J. van den Herik. M.P.H. Huntjens.     #
#                   University of Limburg. Vrije Universiteit                             #
#                                                                                         #
# ####################################################################################### #
...
class Skyler_AI:
    ...
    def three(self, array, colour):  # accepts array 7
        if colour == black:
            ...
            opp = black
        # 0011100 or -1011100 or 001110-1
...
def negate_int(an_int_number):
    return -(an_int_number)

def get_move(b, mymove):
    BOARD_SIZE, BOARD_SIZE = np.shape(b)
    #init boardObject
    boardObject = Board(board=b, size=BOARD_SIZE, AI=Skyler_AI, mymove=mymove)
    for row in range(len(b)):
        for col in range(len(b)):
            if b[row][col] == negate_int(mymove):
                boardObject.opponent_input(row, col)
                b = boardObject.getBoard()
                return b
    return b

class Board:
    def __init__(self, board, size, AI, mymove):
        # Initialise a  board
        self.board = board
        self.size = size
        self.AI = AI()
    ...
    def opponent_input(self, row, col):
        self.play(row, col)
